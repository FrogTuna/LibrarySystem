...
namespace Assignment_1
{
    interface iToolLibrarySystem
    {
        ...
        void add(Tool tool, int num); //add new pieces of an existing tool to the system

        void delete(Tool tool); //delte a given tool from the system

        void delete(Tool tool, int count); //remove some pieces of a tool from the system
        ...
        void borrowTool(Member aMember, Tool tool); //a member borrows a tool from the tool library

        void returnTool(Member aMember, Tool tool); //a member return a tool to the tool library
